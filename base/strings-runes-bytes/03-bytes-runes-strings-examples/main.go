...
func main() {
	...
	/*
		这里没有 UTF-16！​ 所有字符都是用 UTF-8 编码：
		Y, g, e, n, 空格：1 字节​ (ASCII 范围)
		ū：2 字节​ (U+016B 在拉丁扩展区)
		☯：3 字节​ (U+262F 在符号区)
		💀：4 字节​ (U+1F480 表情符号，超出基本多语言平面)
	*/
	str := "Yūgen ☯ 💀"

	fmt.Println("分析 'ū' 和 'ü':")
	fmt.Printf("ū -> Unicode: U+%04X, UTF-8 字节数: %d\n", 'ū', len("ū"))
	fmt.Printf("ü -> Unicode: U+%04X, UTF-8 字节数: %d\n", 'ü', len("ü"))
	fmt.Printf("ü -> Unicode: U+%04X, UTF-8 字节数: %d\n", 'ü', len("💀"))
